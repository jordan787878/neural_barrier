...
def train_barrier_ibp_test(const, net, iterations=20000):
    optimizer = torch.optim.Adam(net.parameters(), lr=1e-3)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1000, gamma=0.95)
    batch_size = 500
    ...
    for iter in range(iterations):
        ...
        x = x_full[outside_goal]
        V = net(x)
        vio_dict["[info] V(not in x_goal) min"] = V.min().item()
        violate = (V <= const.BETA_S)
        violate_percent = 100.0*(violate.sum()/V.shape[0])
        vio_dict["goal"] = violate_percent.item()
        loss_goal_out = torch.mean(torch.relu(const.BETA_S - V))
        loss = loss + loss_goal_out

        x = const.sample_x(const.X_GOAL_RANGE, batch_size)
        x = torch.cat((x, grid_points_goal), dim=0)
        V = net(x)
        vio_dict["[info] V(x_goal) min"] = V.min().item()
        # hinge‐loss to keep 0 ≤ V_min ≤ const.BETA_S
        loss_lower = torch.mean(torch.relu(0.0 - V.min()))             # penalizes V < 0
        loss_upper = torch.mean(torch.relu(V.min() - const.BETA_S))    # penalizes V > const.BETA_S
        loss_goal = loss_lower + loss_upper
        loss = loss + loss_goal
...
def main():
    ...
    if(train_flag):
        ...
        net = train_barrier_ibp_test(const, net, iterations=50000)
